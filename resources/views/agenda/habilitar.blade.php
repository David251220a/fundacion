    ...
    <div class="layout-px-spacing mt-4">
        ...
        <div class="row layout-spacing">
            <div class="col-lg-12 col-md-12 col-sm-12">
                <h2 class="">Habilitacion de Curso Agendado</h2>
                <h3 class="">Familia: {{$curso->familia->descripcion}}</h3>
                <h3 class="">Curso: {{$curso->descripcion}} - {{$curso->modulo->descripcion}}</h3>
            </div>
        </div>
        <form action="{{route('agenda.habilitar_post', $curso)}}" method="POST" enctype="multipart/form-data" onsubmit="disableButton()">
            @csrf
            <div class="row">
                        ...
                        <div class="widget-content widget-content-area">
                                <div class="form-row">
                                    ...
                                    <div class="form-group col-md-3">
                                        ...
                                        <input type="text" name="precio" id="precio" class="form-control text-right" value="{{old('precio')}}" onkeyup="punto_decimal(this)">
                                    </div>
                                    <div class="form-group col-md-3">
                                    ...
                                    <div class="form-group col-md-3">
                                        ...
                                        <select name="instructor_id" id="instructor_id" class="form-control">
                                            @foreach ($instructor as $item)
                                                <option {{ (old('instructor_id') == $item->id ? 'selected' : '' ) }} value="{{$item->id}}">
                    ...
                    <table id="zero-config" class="table dt-table-hover" style="width:100%">
                        <thead>
                            <tr>
                                <th colspan="6">
                                    Seleccionar Todos
                                    <input type="checkbox" name="todos" id="todos" class="" {{ (old('todos') ? 'checked' : '' ) }}  onclick="activar()" value="{{old('todos', 0)}}">
                                </th>
                            </tr>
                            <tr>
                                <th>Inscribir</th>
                        ...
                        <tbody>
                            @foreach ($curso->agendado as $item)
                                <tr>
                                    <td>
                                        <input type="checkbox" name="inscribir[]" id="inscribir[]" class="completo" {{ (old('inscribir') ? 'checked' : '' ) }}>
                                        <input type="hidden" name="alumno_id[]" id="alumno_id[]" value="{{$item->alumno_id}}">
                                    </td>
            ...
            <button type="submit" class="btn btn-success mt-3" id="submitBtn" onclick="this.disabled = true; this.form.submit();">Habilitar</button>
        </form>
    </div>
...
@section('js')
    ...
    <script>
        ...
        function activar()
        {
            let completo = document.getElementById('todos').value;
            let valor = false;
            ...
            var checkboxes = document.querySelectorAll(".completo");
