...
class CertificadoDeuda extends Component
{
    ...
    public $guardando = false, $cursoAlumno, $search, $documento_modal, $nombre_modal, $nombre_id='recibo_comprobante_certificado', $deudacertificado, $deudaexamen, $examen;
    public $titulo_cobro= 'Cobro de Certificado';
    ...
    public function render()
    {
        ...
        $baseQuery = CursoAlumno::query()
            ->whereIn('curso_a_estado_id', [1, 2, 3, 7])
            ->where('estado_id', 1)
            ->where(function ($q) {
                $q->where('certificado_saldo', '>', 0)
                ->orWhere('examen_saldo', '>', 0);
            });

        if (!empty($this->search)) {
            $persona = Persona::where('documento', str_replace('.', '', $this->search))->first();

            if (!empty($persona) && !empty($persona->alumno)) {
                $baseQuery->where('alumno_id', $persona->alumno->id);
            }
        }
        ...
        $aux = (clone $baseQuery)->get();

        // Si querés sumar solo certificado:
        $this->deudacertificado = number_format($aux->sum('certificado_saldo'), 0, ".", ".");

        // (Opcional) si querés también la deuda de examen:
        $this->deudaexamen = number_format($aux->sum('examen_saldo'), 0, ".", ".");
    ...
    {   
        ...
        $this->titulo = 'Cobro de Certificado'; 
        $this->examen = 0;

    }

    public function detalle_examen(CursoAlumno $cursoAlumno)
    {
        $this->documento_modal = number_format($cursoAlumno->alumno->persona->documento, 0, ".", ".");
        $this->nombre_modal = $cursoAlumno->alumno->persona->nombre . ' ' . $cursoAlumno->alumno->persona->apellido;
    ...
    public function save_examen()
    {
        ...
        $cursoAlumno->examen_pagado = $cursoAlumno->examen_pagado + $total;
        $cursoAlumno->examen_saldo = $cursoAlumno->examen_saldo - $total;
        $cursoAlumno->modif_user_id = auth()->user()->id;
        $cursoAlumno->update();
